# This script uses SMOTE on the qclimate1 response variable, collapsed into a binary outcome
...
# Use a two step cross-validation 
# First phase use a course grid for penalty and mixture
# Select best mixture
# Then tune penalty at best mixture
...
# Now tune penalty at best mixture
...
clusters <- get_cluster_labels(hclust_result, min_cor = 0.7)
...
######## Test model
# Significance testing of predictors - conditional, using drop1
form <- as.formula(
  paste(
    "response_binary ~",
...
extract_question_level <- function(var, varlab, vallab){
  
  question_level <- strsplit(var, "_X")
  question <- question_level[[1]][1]
  level <- question_level[[1]][2]
  question_txt <- varlab$label[which(varlab$variable == question)]
  level_txt <-  vallab$label[which(vallab$variable == question & vallab$value == level )]
  return(list(question_txt, level_txt))
}


# We want to see which responses correlate so
# we extract the question-answer (level) in each cluster
clusters_grp <- clusters %>% 
  # return a list of question level for each variable
  mutate(q_lev = map(variable, ~extract_question_level(.x, varlab, vallab))) %>% 
  select(-variable) %>% 
  #create nested list of list(q, level)
  group_by(cluster) %>% 
  nest(data =  q_lev) %>% 
  ungroup() %>% 
  # for some clusters, questions are correlated with the other level of response
  # This could occur for variables in which the remaining levels of one-hot encoding are anticorrelated
  # To omit these, we extract teh unique questions by creating a vector of the questions
   # then  extract the length of unique questions
  mutate(unique_q = map_int(data, function(x){
    qs <- map_chr(x$q_lev, ~.x[[1]])
    length(unique(qs))} ) )%>% 
  filter(unique_q > 1) %>% 
  mutate(n = map_int(data, ~length(.x[[1]]))) %>% 
  filter(n > 1) %>% 
  # convert the q_lev list into a string 
  mutate(q_lev_txt = map(data, function(x){
    map(x$q_lev, ~paste(.x, collapse = "_"))
  }))
